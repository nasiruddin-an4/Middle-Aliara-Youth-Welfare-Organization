...
export default function MemberDetailPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        if (mData.success) {
          const found = mData.data.find(
            (m) => m.memberId == memberId || m.id == memberId,
          );
          setMember(found);
        }
        if (pData.success) {
          setAllPayments(pData.data);
        }
      } catch (e) {
        console.error(e);
      } finally {
  ...
  const { memberPayments, totalPaid, paidMonths, byYear } = useMemo(() => {
    if (!member)
      return { memberPayments: [], totalPaid: 0, paidMonths: 0, byYear: {} };
    const payments = allPayments.filter(
      (p) => p.memberId == member.memberId || p.memberId == member.memberId,
    );
      ...
      <div className="max-w-2xl mx-auto px-4 -mt-5">
        ...
        <div className="grid grid-cols-3 gap-3 mb-6">
          {[
            ...
            <div
              key={s.label}
              className={`${s.color} rounded-2xl p-4 text-center border shadow-sm`}
            >
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-5 mb-6 shadow-sm">
          ...
          <div className="flex gap-1.5">
            {MONTH_NAMES.map((_, i) => {
              const hasPaid = Object.values(byYear).some(
                (y) => y.months[i + 1],
              );
              return (
                <div
                    ...
                    >
                      ...
                      {paid ? (
                        <>
                          ...
                          {st && (
                            <span
                              className={`inline-flex items-center gap-1 text-[8px] font-semibold px-1.5 py-0.5 rounded-full mt-1 ${st.bg} ${st.text}`}
                            >
                              <span
